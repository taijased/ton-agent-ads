...
export class InMemoryDealExternalThreadRepository implements DealExternalThreadRepository {
  ...
  public async create(
    input: CreateDealExternalThreadInput,
  ): Promise<DealExternalThread> {
    const existing = this.threads.find(
      (entry) =>
        entry.platform === input.platform && entry.chatId === input.chatId,
    );
