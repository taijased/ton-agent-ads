...
export class DealService {
  ...
  private selectAdminContact(channel: Channel): string | null {
    ...
    const preferredLink = channel.contacts.find(
      (contact: Channel["contacts"][number]) => contact.type === "link" && contact.isAdsContact
    ) ?? channel.contacts.find((contact: Channel["contacts"][number]) => contact.type === "link");

    if (preferredLink === undefined) {
      return null;
    }

    const linkMatch = preferredLink.value.match(/(?:t\.me|telegram\.me)\/([A-Za-z0-9_]{5,})/i);
  ...
  public async updateDealStatus(
    ...
    const allowedTransitions: Record<DealStatus, DealStatus[]> = {
      ...
      rejected: [],
      admin_outreach_pending: ["admin_contacted", "negotiating", "terms_agreed", "awaiting_user_approval", "failed"],
      admin_contacted: ["admin_outreach_pending", "negotiating", "terms_agreed", "awaiting_user_approval", "failed"],
  ...
  private async startAdminOutreach(deal: Deal): Promise<DealActionResult> {
    ...
    if (adminContact === null || adminContact.trim().length === 0) {
      await this.dealRepository.updateDealStatus(deal.id, {
        status: deal.status,
        outreachError: "Channel contact could not be determined from parsed data",
  ...
  private buildOutreachMessage(input: {
    ...
    const intro = normalizedDescription.includes("ads") || normalizedDescription.includes("реклама")
      ? `Hello! We found ${input.channelTitle} and saw that advertising requests are handled here.`
      : normalizedDescription.includes("promo") || normalizedDescription.includes("collab") || normalizedDescription.includes("сотруднич")
